  ...
  describe('/auth/refresh (POST)', () => {
    it('should preserve remember status', async () => {
      ...
      const newRefreshCookie = Array.isArray(refreshCookies)
        ? refreshCookies.find((c) => c.startsWith('refresh_token'))
        : undefined;
      ...
      const rememberCookie = Array.isArray(loginCookies)
      ...
      const newRememberCookie = Array.isArray(refreshCookies2)
        ? refreshCookies2.find((c) => c.startsWith('refresh_token'))
        : undefined;
