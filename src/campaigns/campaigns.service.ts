...
export class CampaignsService {
    ...
    async update(id: string, updateCampaignDto: UpdateCampaignDto, brandId: string) {
        ...
        if (campaign.brand.id !== brandId) {
            this.logger.warn(`Forbidden: Brand ${brandId} attempted to update campaign ${id} belonging to ${campaign.brand.id}`);
            throw new ForbiddenException('You can only update your own campaigns');
        }

        if (campaign.status === CampaignStatus.DELETED) {
            this.logger.warn(`Forbidden: Brand ${brandId} attempted to update deleted campaign ${id}`);
    ...
    async apply(campaignId: string, creator: CreatorProfile, message?: string) {
        this.logger.log(`Processing application from creator ${creator.id} for campaign ${campaignId}`);
        const campaign = await this.findOne(campaignId);

        if (campaign.status !== CampaignStatus.ACTIVE) {
            this.logger.warn(`Forbidden: Creator ${creator.id} attempted to apply to inactive campaign ${campaignId}`);
        ...
        if (existingApplication) {
            this.logger.warn(`Forbidden: Creator ${creator.id} already applied to campaign ${campaignId}`);
        ...
        this.logger.log(`Successfully created application ${saved.id} for creator ${creator.id} on campaign ${campaignId}`);
        return saved;
    }
    ...
    async delete(id: string, brandId: string) {
        ...
        if (campaign.brand.id !== brandId) {
            this.logger.warn(`Forbidden: Brand ${brandId} attempted to delete campaign ${id} belonging to ${campaign.brand.id}`);
